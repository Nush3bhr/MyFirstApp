...
const Tweet = (props) => {
  const [isMessage, changeMessage] = useState("Please SMILE n LIKE");
  const [isWater, setWater] = useState(0);

  const newMessage = () => {
    changeMessage("You are not a Great Professor");
  };
  const drankWater = () => {
    setWater(isWater + 1);
  };
    ...
    <Grid container>
      <Grid item xs={8}>
        <div>
          ...
          <p>{isMessage}</p>
          <Button variant="contained" color="primary" onClick={newMessage}>
            LIKE
          </Button>
          <p className="fonstyle">THIRSTY??</p>
          <Button variant="contained" color="primary" onClick={drankWater}>
            Drink n Click!!
          </Button>
